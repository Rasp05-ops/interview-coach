...
export default function AgentPage() {
  ...
  if (!sid) return (
    <main className="max-w-6xl mx-auto pb-12">
      <header className="flex items-center justify-between mb-12">
        <a href="/" className="flex items-baseline gap-2"><span className="display-face text-lg text-ink">Interview</span><span className="text-[11px] tracking-[0.24em] text-dim">COACH</span></a>
      ...
      <div className="grid lg:grid-cols-[.8fr_1.2fr] gap-8 items-start">
        ...
        <section className="bg-card border border-border rounded-[28px] p-5 sm:p-8 shadow-[0_20px_60px_rgba(33,76,58,0.08)]">
          ...
          <div className="grid sm:grid-cols-2 gap-4 mb-6"><label className="block"><span className="text-xs font-bold uppercase tracking-[0.12em] text-muted">Role *</span><input className="mt-2 w-full bg-surface border border-border rounded-xl px-4 py-3 text-sm text-ink placeholder:text-dim focus:outline-none focus:border-accent focus:ring-4 focus:ring-accent/10" value={role} onChange={e => setRole(e.target.value)} placeholder="Software Engineer" /></label><label className="block"><span className="text-xs font-bold uppercase tracking-[0.12em] text-muted">Company</span><input className="mt-2 w-full bg-surface border border-border rounded-xl px-4 py-3 text-sm text-ink placeholder:text-dim focus:outline-none focus:border-accent focus:ring-4 focus:ring-accent/10" value={company} onChange={e => setCompany(e.target.value)} placeholder="Company name" /></label></div>
          <label className="block mb-6"><span className="text-xs font-bold uppercase tracking-[0.12em] text-muted">Company website <span className="font-normal text-dim">(optional)</span></span><input className="mt-2 w-full bg-surface border border-border rounded-xl px-4 py-3 text-sm text-ink placeholder:text-dim focus:outline-none focus:border-accent focus:ring-4 focus:ring-accent/10" value={companyUrl} onChange={e => setCompanyUrl(e.target.value)} placeholder="https://company.com" /></label>
          <div className="space-y-5">
            <div><div className="flex items-center justify-between mb-2"><span className="text-xs font-bold uppercase tracking-[0.12em] text-muted">Resume</span><span className="text-xs text-dim">PDF · DOCX · TXT</span></div><label className={`block border-2 border-dashed rounded-2xl p-5 text-center cursor-pointer transition-colors ${resume ? "border-success/40 bg-success-dim/40" : "border-border hover:border-accent/50 bg-surface"}`}><input type="file" className="hidden" accept=".pdf,.docx,.txt" onChange={e => upload(e.target.files?.[0], "resume")} />{fileNote.resume ? <span className="text-sm text-success">{fileNote.resume}</span> : <span className="text-sm text-muted">Drop your resume here or browse files</span>}</label><textarea className="mt-3 w-full h-32 bg-surface border border-border rounded-xl p-3 text-sm text-ink placeholder:text-dim focus:outline-none focus:border-accent focus:ring-4 focus:ring-accent/10" value={resume} onChange={e => setResume(e.target.value)} placeholder="Your extracted resume text will appear here…" /></div>
            <div><div className="flex items-center justify-between mb-2"><span className="text-xs font-bold uppercase tracking-[0.12em] text-muted">Job description</span><span className="text-xs text-dim">Optional, but useful</span></div><label className="block border border-border rounded-2xl px-4 py-3 cursor-pointer hover:border-accent/50 bg-surface"><input type="file" className="hidden" accept=".pdf,.docx,.txt" onChange={e => upload(e.target.files?.[0], "jd")} /><span className="text-sm text-muted">{fileNote.jd || "Attach a JD or paste it below"}</span></label><textarea className="mt-3 w-full h-24 bg-surface border border-border rounded-xl p-3 text-sm text-ink placeholder:text-dim focus:outline-none focus:border-accent focus:ring-4 focus:ring-accent/10" value={jd} onChange={e => setJd(e.target.value)} placeholder="What does this role need?" /></div>
          </div>
          <div className="flex flex-wrap gap-4 items-center mt-6 pt-5 border-t border-border text-sm text-muted"><label className="flex items-center gap-2">Evaluation <select className="bg-surface border border-border rounded-lg px-2 py-1.5 text-xs" value={mode} onChange={e => setMode(e.target.value as any)}><option value="inline">Inline</option><option value="async">Async</option></select></label><label className="flex items-center gap-2"><input className="accent-accent" type="checkbox" checked={agenticRag} onChange={e => setAgenticRag(e.target.checked)} /> Adaptive retrieval</label></div>
  ...
  return (
    <main className="max-w-5xl mx-auto pb-12">
      <header className="flex items-center justify-between mb-8"><a href="/" className="flex items-baseline gap-2"><span className="display-face text-base text-ink">Interview</span><span className="text-[10px] tracking-[0.24em] text-dim">COACH</span></a><span className="flex items-center gap-2 text-xs text-muted"><span className={`w-2 h-2 rounded-full ${socketReady ? "bg-success" : "bg-warning"}`} />{socketReady ? "Room connected" : "Connecting"}</span></header>
      ...
      {msgs.map((m, i) => (
        <div key={i} className={`rounded-[22px] p-5 text-sm border mb-3 shadow-[0_10px_30px_rgba(33,76,58,0.05)] ${m.who === "agent" ? "bg-card border-border" : "bg-accent-dim border-accent/20 ml-8"}`}>
      ...
      <section className="rounded-[22px] border border-border bg-forest p-5 space-y-4 shadow-[0_18px_45px_rgba(33,76,58,0.16)]">
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <p className="text-xs text-muted mt-1">{socketReady ? "Your room is listening for a clear ending." : "Connecting to the realtime engine…"}</p>
          </div>
        ...
        {interim && <p className="text-sm text-ink border-t border-white/10 pt-3">{interim}</p>}
      </section>
      {!done ? (
        <div className="flex gap-2 mt-3">
          <textarea className="flex-1 bg-card border border-border rounded-xl p-3 text-sm h-20 focus:outline-none focus:border-accent focus:ring-4 focus:ring-accent/10" value={draft} onChange={e => setDraft(e.target.value)} placeholder="Or type your answer here…" />
        ...
        <div className="rounded-[22px] border border-border bg-card p-5 text-sm space-y-2 mt-3">
